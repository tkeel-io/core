...
func condition2boolQuery(conditions []*pb.SearchCondition, boolQuery *elastic.BoolQuery) {
	for _, condition := range conditions {
		...
		case "$prefix":
			boolQuery = boolQuery.Must(elastic.NewPrefixQuery(condition.Field, condition.Value.GetStringValue()))
		case "$wildcard":
			boolQuery = boolQuery.Must(elastic.NewWildcardQuery(condition.Field, "*"+condition.Value.GetStringValue()+"*"))
		default:

...
func (n *Node) FlushEntity(ctx context.Context, en Entity) error {
	...
	if err := n.resourceManager.Repo().PutEntity(ctx, en.ID(), en.Raw()); nil != err {
		log.L().Error("flush entity state storage", zap.Error(err), logf.Eid(en.ID()))
	...
	if _, err := n.resourceManager.Search().IndexBytes(ctx, en.ID(), globalData); nil != err {
		log.L().Error("flush entity search engine", zap.Error(err), logf.Eid(en.ID()))
	...
	if err := n.flushTimeSeries(ctx, en); nil != err {
		log.L().Error("flush entity timeseries database", zap.Error(err), logf.Eid(en.ID()))
	}

	// 4. flush raw data.
	if err := n.flushRawData(ctx, en); nil != err {
		log.L().Error("flush entity rawData", zap.Error(err), logf.Eid(en.ID()))
	}
	return nil
}
...
func (n *Node) RemoveEntity(ctx context.Context, en Entity) error {
		...
		if nil != err {
			if innerErr := n.FlushEntity(ctx, en); nil != innerErr {
				log.L().Error("remove entity failed, recover entity state failed", logf.Eid(en.ID()),
					logf.Reason(err.Error()), zap.Error(innerErr), logf.Value(string(en.Raw())))
			}
		}
	}()
	...
	if err := n.resourceManager.Repo().
		...
		log.L().Error("remove entity from state storage",
			zap.Error(err), logf.Eid(en.ID()), logf.Value(string(en.Raw())))
